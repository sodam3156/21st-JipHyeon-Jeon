...
- 메인 아이디어는 `D-010`(2026-07-25)으로 **`IDEA-002 돌봄 집현전`(SOL-002, OPP-005)이 확정**되었습니다. 제품 책임자가 선택 시한을 근거로 Gate 0–5 정식 통과 전에 직접 확정한 D-006의 명시적 예외이며, 관련 Gate는 `governance.gate-log`에 `waived`(만료 2026-08-11)로 기록되어 있습니다.
- 현재 단계는 **PRD·문제·고객 정본 채움과 Gate waiver의 실제 근거 전환**(이슈 #3 48시간 검증, #4 공식 데이터 스파이크)입니다.
- `회동(會同)` `IDEA-001`과 비교군 OPP-001–003은 D-010 이후 `parked`입니다. 폐기가 아니며 판정 근거는 `governance.gate-log`에 유지됩니다.
- 이슈 #3·#4가 실패 기준을 충족하면 D-010을 superseded 처리하고 재검토합니다.
- 세부 현황의 정본은 `planning.current-status`입니다. 이 절과 어긋나면 그쪽이 맞습니다.
