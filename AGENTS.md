...
- 메인은 `D-011`(2026-07-29)로 **Make Sejong Great Again**(운영자 승인형 다교차로 교통제어 데모)입니다. 구현 정본은 GitHub `sodam3156/Make-sejong-great-again`, 리서치·부스·발표 산출물은 vault `sodam3156/sejong-jiphyeonjeon`에 있습니다.
- `IDEA-002 돌봄 집현전`(SOL-002)은 D-010으로 선택됐다가 **D-011로 폐지**됐습니다. 이 저장소의 문제·고객·솔루션·PRD 정본 4종은 `archived` 이력으로 보존합니다.
- 이 저장소에 MSGA의 문제·고객·PRD 정본을 재작성할지는 미정입니다. 그 전까지 이 저장소는 결정 이력과 거버넌스 규칙의 정본입니다.
- `회동(會同)` `IDEA-001`과 비교군 OPP-001–003은 `parked` 이력입니다.
