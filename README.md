...
**메인 아이디어는 `IDEA-002 돌봄 집현전`(SOL-002)으로 확정했습니다.**
근거: `D-010`에서 제품 책임자가 2026-07-25 채택했으며, 현재는 `waived` 상태의 예외 경로 하에 `issue #3`·`issue #4` 결과로 보강 중입니다.

- 현재 단계: 메인 아이디어 확정 + PRD/문제/고객 정본 보강
- 1차 비교 영역: 정시 무차량 이동, 문화 수요 기반 콘텐츠 실행, 대형행사 분산 귀가 (`OPP-001`~`OPP-003`)는 D-010 이후 `parked`
- 기존 `회동(會同)`은 `IDEA-001`, `seed` 가설 메모로 보관되며, 재검토는 가능
- 이전 솔루션 문서는 archived로 보관, 메인 아이디어의 문제·고객·PRD 정본은 현재 채우는 중
- 메인 아이디어 채택은 2026-08-11 전 이슈 #3/4 결과로 waiver 전환 여부를 검토합니다
